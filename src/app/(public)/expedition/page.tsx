...
const defaultExpeditionData = {
  ...
  itinerary: [
    {
      day: "DAY 1", title: "Departure & Secret Islands", image: imgVessel,
      activities: [
        { time: "Morning Pick-Up", text: "Lombok Hotel Pick-Up (08:00 - 10:00 AM) via Toyota Hiace/Elf to Kopang office.", iconName: "sunrise" },
        { time: "03:30 PM", text: "Board the KM Pulau Mas 88 and depart towards Komodo National Park.", iconName: "anchor" },
      ...
      meals: "Dinner Included", overnight: "Overnight onboard while sailing."
    },
    {
      day: "DAY 2", title: "Whale Sharks & Tambora", image: imgWhaleShark,
      activities: [
        { time: "Morning", text: "Swim alongside majestic whale sharks in the calm waters of Saleh Bay.", iconName: "sunrise" },
        { time: "Midday", text: "Sail across Tambora waters. Keep an eye out for playful dolphins.", iconName: "sun" },
        { time: "Afternoon", text: "Relax on deck and take in the ocean views while cruising.", iconName: "sunset" }
      ],
      meals: "Breakfast, Lunch & Dinner", overnight: "Overnight onboard."
    },
    {
      day: "DAY 3", title: "Komodo & Padar Excursion", image: imgKomodo,
      activities: [
        { time: "Morning", text: "Guided trekking at Loh Liang Village. Explore the habitat of Komodo dragons.", iconName: "sunrise" },
    ...
    {
      day: "DAY 4", title: "Majarite, Kelor & Return", image: imgPinkBeach,
      activities: [
        { time: "Morning", text: "Snorkeling session at Majarite Island. Explore colorful coral reefs.", iconName: "sunrise" },
  ...
  cabinPackages: [
    { name: "Private Cabin Sea View", desc: "A premium cabin option with ocean views.", price: "4,600K", features: ["Sea view window", "Comfortable area", "Air-conditioned"], image: "https://images.unsplash.com/photo-1506012787146-f92b2d7d6d96?q=80&w=2069&auto=format&fit=crop", popular: true },
    { name: "Private Cabin Standard", desc: "Comfortable private room for extra privacy.", price: "4,200K", features: ["Extra privacy", "Comfortable bed", "Air-conditioned"], image: "https://images.unsplash.com/photo-1520250497591-112f2f40a3f4?q=80&w=2070&auto=format&fit=crop", popular: false },
    { name: "Down Deck Cabin", desc: "Cozy lower-deck cabin with a comfortable sleeping area.", price: "3,800K", features: ["Cozy and quiet", "Comfortable space", "AC Central"], image: "https://images.unsplash.com/photo-1516690561799-46d8f74f9abf?q=80&w=2070&auto=format&fit=crop", popular: false },
  ...
  inclusions: ["Pick-up service", "Meals onboard", "Snacks & drinks", "Snorkeling equipment", "Safety equipment", "Government tax", "English-speaking guide"],
  ...
  faqs: [
    ...
    { q: "When should I book?", a: "Book at least one week before departure to secure your spot." },
    { q: "Is the trip safe?", a: "Yes, we prioritize safety with regular boat checks and experienced crew." }
  ]
};
...
export default function ExpeditionPage() {
    ...
    const fetchExpeditionData = async () => {
      try {
        ...
        if (docSnap.exists()) {
          ...
          const mergedItinerary = data.itinerary && Array.isArray(data.itinerary) && data.itinerary.length > 0 ? data.itinerary.map((it: any, i: number) => ({...defaultExpeditionData.itinerary[i % defaultExpeditionData.itinerary.length], ...it, image: it.image || defaultExpeditionData.itinerary[i % defaultExpeditionData.itinerary.length].image, activities: Array.isArray(it.activities) ? it.activities.map((act: any) => ({time: act.time || "00:00", text: act.text || "", iconName: act.iconName || "sun"})) : []})) : defaultExpeditionData.itinerary;
  ...
  const getWaLink = (cabinName: string) => `https://wa.me/${waNumber}?text=Hi%20PMM%20Voyage,%20I%20am%20interested%20in%20booking%20the%20*${encodeURIComponent(cabinName || "Cabin")}*%20for%20the%204D3N%20Expedition.`;
  ...
  return (
    <main className="flex flex-col w-full bg-[#f8f9fa] overflow-x-hidden">
        ...
        <div className="max-w-7xl mx-auto w-full relative z-10 flex flex-col lg:flex-row items-center justify-center gap-12 lg:gap-8">
          ...
          >
            <motion.div variants={fadeInUp} className="inline-flex items-center justify-center gap-2 px-4 py-2 lg:px-5 lg:py-2.5 rounded-full bg-white/5 border border-white/10 text-[#eaddbd] text-[10px] md:text-xs font-bold mb-6 backdrop-blur-md shadow-sm uppercase tracking-widest mx-auto lg:mx-0">
              <Navigation className="h-3.5 w-3.5 text-[#B88E52]" /> The Grand Route
            </motion.div>
            ...
            <motion.h1 variants={fadeInUp} className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-bold text-white mb-4 md:mb-6 tracking-tight leading-[1.15]">
              4D3N Ultimate <br className="hidden sm:block" /> 
            ...
            >
              ...
              <div className="absolute bottom-5 left-5 text-left">
                ...
                <span className="text-white font-bold text-xl tracking-wider">Padar Island</span>
              </div>
            </motion.div>
            ...
            <motion.div variants={fadeInUp} className="flex flex-col sm:flex-row items-center justify-center lg:justify-start gap-3 md:gap-4 w-full px-5 lg:px-0">
              <a 
                ...
                className="px-6 py-4 md:px-8 md:py-4 rounded-full bg-[#B88E52] hover:bg-[#a37c46] text-white font-bold text-base md:text-lg transition-all shadow-[0_4px_20px_rgba(184,142,82,0.3)] hover:-translate-y-1 flex items-center justify-center gap-2 w-full sm:w-auto"
              >
                Book This Voyage <ArrowRight className="w-4 h-4 md:w-5 md:h-5" />
              </a>
              <button 
                onClick={() => document.getElementById('itinerary')?.scrollIntoView({ behavior: 'smooth' })}
                className="px-6 py-4 md:px-8 md:py-4 rounded-full bg-white/5 hover:bg-white/10 border border-white/20 text-white font-bold text-base md:text-lg transition-all flex items-center justify-center gap-2 w-full sm:w-auto"
              >
                View Itinerary
              </button>
            </motion.div>
      ...
      {/* 2. TRIP HIGHLIGHTS (Horizontal Scroll on Mobile, Bento Grid on Desktop) */}
      <section className="py-16 md:py-24 px-5 md:px-12 bg-white relative z-20">
        <div className="max-w-7xl mx-auto">
          ...
          >
            <motion.h2 variants={fadeInUp} className="text-3xl md:text-4xl lg:text-5xl font-bold text-[#0f172a] mb-4 md:mb-6">Expedition Highlights</motion.h2>
          ...
          <motion.div 
            ...
            viewport={{ once: true }}
            variants={staggerContainer}
            // Mobile: Horizontal Scroll. Desktop: CSS Grid Bento
              ...
              return (
                ...
                >
                  ...
                  <div className="absolute bottom-6 left-6 right-6 md:bottom-8 md:left-8 md:right-8 text-white">
                    <h3 className="text-xl md:text-2xl lg:text-3xl font-bold mb-2 leading-tight">{item.title}</h3>
      ...
      <section id="itinerary" className="py-16 md:py-24 px-5 md:px-12 bg-[#f8f9fa] border-y border-gray-100 relative">
        <div className="text-center mb-12 md:mb-20 relative z-10">
          <span className="text-[#B88E52] font-semibold tracking-wider uppercase text-xs md:text-sm mb-2 md:mb-3 block">Day by Day</span>
          <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold text-[#0f172a] mb-4 md:mb-6">The Grand Itinerary</h2>
          <p className="text-gray-600 max-w-2xl mx-auto text-base md:text-lg leading-relaxed">Our trips are safe, well-organized, and perfectly paced. Follow the timeline of your upcoming adventure.</p>
        </div>
              ...
              <motion.div variants={fadeInUp} className="w-full md:w-1/2">
                <div className="relative rounded-[1.5rem] md:rounded-[2.5rem] overflow-hidden shadow-2xl aspect-[4/3] group md:sticky md:top-32 border border-gray-200">
                  ...
                  <div className="absolute bottom-5 left-5 right-5 md:bottom-8 md:left-8 md:right-8 text-white">
                    ...
                    <h3 className="text-2xl md:text-3xl font-bold leading-tight">{item.title}</h3>
                  </div>
                </div>
              </motion.div>
        ...
        <div className="max-w-6xl mx-auto">
          ...
          >
            ...
            <motion.div variants={fadeInUp} className="bg-[#fdfaf5] rounded-[1.5rem] md:rounded-[2.5rem] p-6 md:p-10 lg:p-12 border border-[#B88E52]/20 shadow-xl shadow-[#B88E52]/5 relative overflow-hidden">
              ...
              <h3 className="text-2xl md:text-3xl font-bold text-[#0f172a] mb-6 md:mb-8 flex items-center gap-2 md:gap-3">
      ...
      <section className="py-16 md:py-24 px-5 md:px-12 bg-[#0f172a] text-white">
        <div className="max-w-7xl mx-auto">
          <motion.div 
            ...
            className="text-center mb-10 md:mb-16"
          >
            <motion.span variants={fadeInUp} className="text-[#B88E52] font-semibold tracking-wider uppercase text-xs md:text-sm mb-2 block">Accommodation</motion.span>
            <motion.h2 variants={fadeInUp} className="text-3xl md:text-4xl lg:text-5xl font-bold mb-4 md:mb-6">Choose Your Stay</motion.h2>
              ...
              return (
                ...
                >
                  <div className="h-48 md:h-56 overflow-hidden relative">
                    ...
                    {cabin.popular && (
                      <div className="absolute top-3 right-3 md:top-4 md:right-4 bg-[#B88E52] text-white text-[10px] font-bold px-3 py-1 md:px-4 md:py-1.5 rounded-full uppercase tracking-wider shadow-lg">Most Popular</div>
                    )}
                    <div className="absolute bottom-4 left-4 md:bottom-5 md:left-5 text-white pr-4">
                       <h3 className="text-lg md:text-xl font-bold leading-tight">{safeCabinName}</h3>
                    </div>
                  </div>
                  <div className="p-5 md:p-6 flex flex-col flex-grow">
                    ...
                    <div className="mt-auto pt-5 md:pt-6 border-t border-white/10">
                      ...
                      <a 
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="flex items-center justify-center w-full py-3 md:py-4 rounded-xl md:rounded-2xl bg-white text-[#0f172a] font-bold text-sm md:text-base hover:bg-[#B88E52] hover:text-white transition-colors shadow-lg hover:-translate-y-1"
                      >
                        Select Package
                      </a>
                    </div>
        ...
        <section className="py-16 md:py-24 px-5 md:px-12 bg-[#f8f9fa] border-b border-gray-100">
          <div className="max-w-7xl mx-auto text-center">
            <motion.div 
              ...
              variants={staggerContainer}
            >
              <motion.span variants={fadeInUp} className="text-[#B88E52] font-semibold tracking-wider uppercase text-xs md:text-sm mb-2 block">
                Secure Transactions
              </motion.span>
              <motion.h2 variants={fadeInUp} className="text-2xl md:text-4xl lg:text-5xl font-bold text-[#0f172a] mb-8 md:mb-12">
              ...
              <motion.div variants={fadeInUp} className="flex flex-wrap justify-center items-center gap-4 md:gap-10">
                {expData.paymentMethods.map((method: any, idx: number) => (
                  <div key={idx} className="flex flex-col items-center group cursor-pointer w-28 md:w-auto">
                    ...
                    <span className="text-[10px] md:text-xs text-gray-500 mt-2 md:mt-4 font-semibold opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity duration-300 tracking-wide text-center">
                      {method.name}
                    </span>
                  </div>
                ))}
      ...
      <section className="py-16 md:py-24 px-5 md:px-12 bg-white border-t border-gray-100">
        {/* Gunakan order-2 lg:order-1 untuk memindahkan posisi di Mobile */}
        <div className="max-w-7xl mx-auto flex flex-col lg:flex-row gap-12 md:gap-16">
          ...
          >
            <div className="bg-[#fdfaf5] p-6 md:p-8 lg:p-10 rounded-[1.5rem] md:rounded-[2.5rem] border border-[#B88E52]/20 lg:sticky lg:top-32 shadow-lg shadow-[#B88E52]/5">
              <h3 className="text-xl md:text-2xl font-bold text-[#0f172a] mb-6 md:mb-8 flex items-center gap-2 md:gap-3">
              ...
              <div className="mt-8 md:mt-10 pt-6 md:pt-8 border-t border-[#B88E52]/20 text-center">
                <p className="text-xs md:text-sm text-gray-600 mb-3 md:mb-4">Have specific requests or concerns?</p>
                <a 
                  id="btn-wa-support"
                  href={b2cWaLink} 
                  target="_blank" 
                  rel="noopener noreferrer" 
                  className="inline-flex items-center justify-center gap-2 w-full py-3.5 md:py-4 rounded-xl bg-[#0f172a] text-white font-bold text-sm md:text-base hover:bg-[#1e293b] transition-colors shadow-md hover:-translate-y-1"
                >
                   Chat with Support
                </a>
              </div>
          ...
          >
            <h2 className="text-2xl md:text-4xl lg:text-5xl font-bold text-[#0f172a] mb-6 md:mb-10">Trip FAQs</h2>
