    ...
    const fetchArticleBySlug = async () => {
      ...
      try {
        ...
        if (!querySnapshot.empty) {
          ...
          // 2. Fetch Artikel Terkait (Rekomendasi Sidebar)
  ...
  if (!article) {
    return (
      <div className="min-h-[80vh] flex flex-col items-center justify-center bg-[#f8f9fa] pt-20 px-6 text-center">
        ...
        <Link href="/blog" className="px-8 py-3 rounded-full bg-[#B88E52] text-white font-bold shadow-lg hover:bg-[#a37c46] transition-colors">
          Return to Journal
        </Link>
      </div>
    );
  ...
  return (
    <main className="flex flex-col w-full bg-[#f8f9fa] min-h-screen relative">
        ...
        <div className="flex flex-col lg:flex-row gap-8 lg:gap-12">
          ...
          <div className="w-full lg:w-2/3 bg-white rounded-[2rem] md:rounded-[3rem] shadow-[0_20px_50px_rgba(17,34,58,0.05)] p-8 md:p-12 lg:p-16 border border-gray-100">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 mb-12 pb-8 border-b border-gray-100">
              <Link href="/blog" className="inline-flex items-center gap-2 text-gray-500 hover:text-[#B88E52] transition-colors text-sm font-semibold">
              ...
              <div className="flex items-center gap-3">
                ...
                <a href={`https://www.facebook.com/sharer/sharer.php?u=${currentUrl}`} target="_blank" rel="noopener noreferrer" className="w-9 h-9 rounded-full border border-gray-200 flex items-center justify-center text-gray-500 hover:border-[#11223a] hover:bg-gray-50 transition-all">
              ...
              <div className="space-y-6">
                {relatedArticles.length > 0 ? (
                  relatedArticles.map((rel, idx) => (
                    <Link key={idx} href={`/blog/${rel.slug}`} className="group flex gap-4 items-center">
      ...
      <style dangerouslySetInnerHTML={{__html: `
        .article-content { font-family: var(--font-geist-sans), sans-serif; }
        .article-content > p:first-of-type::first-letter { font-size: 3.5rem; font-weight: 800; float: left; line-height: 1; margin-right: 0.5rem; color: #11223a; margin-top: 0.2rem; }
        .article-content p { color: #374151; line-height: 1.8; font-size: 1.125rem; margin-bottom: 1.5rem; }
        .article-content h2 { font-size: 2rem; font-weight: 800; color: #11223a; margin-top: 3rem; margin-bottom: 1.25rem; line-height: 1.3; letter-spacing: -0.02em; }
        .article-content h3 { font-size: 1.5rem; font-weight: 700; color: #11223a; margin-top: 2.5rem; margin-bottom: 1rem; }
        .article-content ul { list-style-type: disc; padding-left: 1.5rem; margin-bottom: 1.5rem; color: #374151; font-size: 1.125rem; line-height: 1.8; }
        .article-content ol { list-style-type: decimal; padding-left: 1.5rem; margin-bottom: 1.5rem; color: #374151; font-size: 1.125rem; line-height: 1.8; }
        .article-content li { margin-bottom: 0.75rem; }
        .article-content blockquote { border-left-width: 4px; border-color: #B88E52; padding-left: 1.5rem; font-style: italic; color: #6b7280; margin: 2.5rem 0; background-color: #fdfaf5; padding: 1.5rem; border-radius: 0 1rem 1rem 0; font-size: 1.25rem; line-height: 1.7; }
        .article-content strong { font-weight: 700; color: #11223a; }
        .article-content img { max-width: 100%; height: auto; border-radius: 1rem; box-shadow: 0 10px 30px rgba(0,0,0,0.1); margin: 3rem auto; display: block; }
      `}} />
