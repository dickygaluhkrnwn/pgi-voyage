...
export default function CreateGalleryMediaPage() {
  ...
  const [localPreview, setLocalPreview] = useState<string>(''); // Untuk preview sebelum upload
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!title || !file || !categoryId || !location) {
      setError('Mohon lengkapi judul, lokasi, kategori, dan pastikan file sudah dipilih.');
      return;
    }
    ...
    try {
      ...
      await addDoc(collection(db, 'galleries'), mediaData);
      
      // Kembali ke halaman gallery list
          ...
          <form onSubmit={handleSubmit} className="bg-white p-8 rounded-3xl border border-gray-100 shadow-sm space-y-6">
            ...
            <div>
              <label className="block text-sm font-bold text-[#11223a] mb-2">
                Pilih File (Foto / Video) <span className="text-red-500">*</span>
              </label>
            ...
            <div className="pt-6 border-t border-gray-100 flex flex-col items-center">
              ...
              <button
                type="submit"
                disabled={isSubmitting || isAuthChecking || !user || !file}
              ...
              >
                ...
                {isAuthChecking ? 'Memeriksa Akses...' : isSubmitting ? 'Memproses...' : 'Simpan Media ke Galeri'}
              </button>
            </div>
          </form>
        </motion.div>
            ...
            <div className={`relative bg-gray-100 rounded-2xl overflow-hidden shadow-inner mx-auto ${type === 'reel' ? 'aspect-[9/16] w-3/4' : 'aspect-[4/3] w-full'}`}>
              {localPreview ? (
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center text-gray-400 p-6 text-center">
                  ...
                  <p className="text-sm font-medium">Pilih file untuk melihat preview</p>
                </div>
              )}
            </div>
